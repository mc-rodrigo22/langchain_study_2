...
def create_assistant():
    if "assistant" not in st.session_state:
        st.session_state.assistant = openai.Assistants.create(
...
def poll_run_status(run_id, thread_id, timeout=30):
    ...
    while time.time() - start_time < timeout:
        run = openai.Threads.runs.retrieve(thread_id=thread_id, run_id=run_id)
        ...
        with st.spinner("Processing your query..."):
            try:
                ...
                if "thread" not in st.session_state:
                    st.session_state.thread = openai.Threads.create(
                        messages=[
                            {
                                "role": "user",
                                "content": query
...
st.sidebar.markdown(
    "[View this project on GitHub](https://github.com/mc-rodrigo22/langchain_study_2)"
)
